...
module.exports = {
  ...
  attachHydrusDatabases () {
    this.hyve.prepare(
      `ATTACH '${config.hydrusClientDbPath}' AS hydrus_client_db`
    ).run()
    this.hyve.prepare(
    ...
    this.hyve.prepare(
      `ATTACH '${config.hydrusCachesDbPath}' AS hydrus_caches_db`
    ).run()
  },
  detachHydrusDatabases () {
    this.hyve.prepare('DETACH hydrus_client_db').run()
    this.hyve.prepare('DETACH hydrus_master_db').run()
    this.hyve.prepare('DETACH hydrus_mappings_db').run()
    this.hyve.prepare('DETACH hydrus_caches_db').run()
  }
}
